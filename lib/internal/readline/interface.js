...
function concatBuffer(context, value) {
  if (context[kLine_buffer]) {
    context[kLine_buffer] += value;
  } else {
    context[kLine_buffer] = value;
  }
}

function *splitLines(string, context) {
  let start = 0;
  let end;
  let increment;

  function getNextEnd() {
    const n = StringPrototypeIndexOf(string, '\n', start);
    const r = StringPrototypeIndexOf(string, '\r', start);
    increment = 2;
    if (r === -1 || (n >= 0 && r > n)) return n;
    if (string[r + 1] === '\n') increment++;
    return r;
  }

  while ((end = getNextEnd()) !== -1) {
    let value = StringPrototypeSubstr(string, start, end - start);
    if (context[kLine_buffer]) {
      value = context[kLine_buffer] + value;
      context[kLine_buffer] = null;
    }
    yield value;
    start = end + increment;
  }

  if (start === 0) concatBuffer(context, string);
  else if (start < string.length)
    concatBuffer(context, StringPrototypeSubstr(string, start));
}
...
class Interface extends InterfaceConstructor {
  ...
  [kNormalWrite](b) {
    ...
    if (newPartContainsEnding) {
      ...
      const lines = splitLines(string, this);

      for (const line of lines) this[kOnLine](line);
      this[kLine_buffer] = lines[kLine_buffer];
    } else if (string) {
      ...
      switch (key.name) {
        ...
        default:
          if (typeof s === 'string' && s) {
            const context = {};
            const lines = splitLines(s, context);
            let first = true;
