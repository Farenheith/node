...
function splitLines(string, keepBuffer, buffer) {
  let start = 0;
  let end = 0;
...
class Interface extends InterfaceConstructor {
  ...
  [kNormalWrite](b) {
    ...
    if (newPartContainsEnding) {
      ...
      const lines = splitLines(string, true, this[kLine_buffer]);
      ...
      switch (key.name) {
        ...
        default:
          if (typeof s === 'string' && s) {
            const lines = splitLines(s, false);
